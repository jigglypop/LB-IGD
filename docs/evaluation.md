...
### 초록
제1–3장은 설계 변수 $x$가 게임(MDP) $\mathcal{M}_x$를 정의하고, 내부(self-play/RL) 최적화로 얻는 정책 $\pi_x^*$의 관측 통계 $y(x;\omega)$를 통해 외부 목적 $J(x)$를 최소화하는 bilevel/black-box 구조를 정식화하였다. 그러나 “승패가 비슷하면 성공인가?”라는 질문에 답하려면, 설계 $x$의 품질을 측정하는 **평가 프로토콜**(학습 예산, 매칭 구성, 시드, 통계 추정 방식)을 고정하고, 성공 기준을 정량화해야 한다.

본 장은 (i) self-play 기반 평가를 재현 가능하게 고정하는 최소 프로토콜을 제시하고, (ii) **승률 기반 밸런스 지표를 중심으로** 성공 판정식을 구성하며, (iii) 노이즈/퇴화 해(degenerate solution)를 배제하기 위한 제약 및 강건성 체크를 정리한다. 분포 기반 메타 지표는 “승률을 보조하는 선택적 지표”로 다룬다(예: 퇴화 해 탐지, 해석 가능성 강화).



### 1. “성공”은 승률만이 아니라, 고정된 프로토콜에서의 목적 $J(x)$ 달성이다
설계의 품질은 “동일한 평가 절차”를 통해 얻는 통계로만 정의되어야 한다. 즉, 평가 절차(학습 예산, 매칭 구성, 시드, 초기화)가 바뀌면 $y(x;\omega)$의 분포도 바뀌므로, 서로 다른 $x$를 공정하게 비교하려면 다음을 **명시적으로 고정**해야 한다.
...
승률 평탄화만으로는 설계가 “재미 없는 50:50”으로 퇴화하는 경우가 있으므로, 보조 지표로 플레이 패턴(예: 교전 거리 분포)을 함께 관찰할 수 있다. 제2장처럼 교전 거리의 경험분포 $\hat p_x(\cdot;\omega)$를 구성하고, 목표 메타 $q^\star$와의 거리를 측정한다. 1차원 경험분포 간 $W_2$는 정렬 기반으로 계산할 수 있다.
...
이 값이 작아지는 것은 “승패 결과”가 아니라 “플레이 패턴(거리대의 리듬)”이 목표 메타에 가까워졌음을 의미한다.
