...
### 초록
강화학습에서 벨만 방정식과 동적계획법(DP)은 **고정된** 마르코프 결정과정(MDP) 위에서 성립한다. 반면 “게임 설계”는 맵, 유닛, 팩션 규칙 등 **MDP 자체를 바꾸는 상위 수준의 최적화 문제**로 귀결된다. 본 장은 (i) 플레이 문제와 설계 문제를 수학적으로 분리하여 정식화하고, (ii) 왜 벨만/DP가 설계 문제의 바깥 레벨에는 적용되지 않는지를 원리 수준에서 보이며, (iii) 설계 문제를 확률적 black-box 최적화 및 bilevel 최적화로 정리한다.

---

### 키워드
MDP, Bellman 방정식, 동적계획법, self-play, bilevel 최적화, 확률적 black-box 최적화, 게임 설계

---

### 표기 및 약속(Notations and Conventions)
이후 장들(`blackbox.md`, `inverse.md`, `evaluation.md`)에서 공통으로 사용하는 표기는 다음과 같다.

- $x \in \mathcal{X}$: 설계 변수(맵/규칙/유닛/팩션 등 환경을 규정하는 파라미터의 집합, 이산·연속 혼합 가능)
- $\mathcal{M}_x$: 설계 $x$가 정의하는 플레이 레벨 MDP
- $\pi_x^*$: $\mathcal{M}_x$에서의(근사) 최적 정책
- $\omega$: 학습/탐색/매칭 샘플링/초기화가 만들어내는 확률적 요인(시드 포함)
- $y(x;\omega)$: self-play로부터 관측되는 통계량(승률, 거리 분포, 게임 길이 등)
- $L(\cdot)$: 관측 통계에 적용되는 손실(밸런스/공정성/제약 포함)
- $J(x) := \mathbb{E}_\omega[L(y(x;\omega))]$: 외부 설계 목적함수(기대 손실)

또한 본 장에서 “벨만/DP의 적용 가능성”은 **고정된 MDP $\mathcal{M}$** 위에서 최적성 원리(재귀적 분해)가 성립하는지 여부를 의미한다.

---

### 문서 구성(파일 분할) 및 범위
본 글은 분량과 논리 전개의 독립성을 위해 4개 파일로 분할되어 있으며, 각 파일의 범위는 다음과 같다.

- `bellman.md`(제1장): 문제 설정, MDP/벨만의 전제, “설계 레벨”에서 DP가 성립하지 않는 구조적 이유, bilevel/블랙박스 최적화로의 재정식화
- `blackbox.md`(제2장): 확률적 블랙박스 목적에서 ES(Score Function Estimator)의 유도, (선택적으로) 분포 정합을 통한 목적함수 설계 예시
- `inverse.md`(제3장): 목표 메타로부터 팩션/맵 파라미터를 역으로 구성하는 inverse design 정식화, 식별성/정규화, 분포기하 관점의 해석
- `evaluation.md`(제4장): self-play 기반 평가 프로토콜, “성공” 판정 기준(승률 중심), 재현성 및 강건성 체크

---

### 1. 서론: “플레이 최적화”와 “설계 최적화”는 다른 문제이다
일반적인 강화학습은 주어진 게임(규칙, 맵, 유닛 스펙)이 고정된 상태에서 최적 정책을 학습하여 **승리 확률을 최대화**한다. 본 장이 다루는 목표는 다르다. 목표는 특정 정책이 게임을 “이기는 방법”이 아니라, 게임의 규칙/맵/구조를 조정하여 **어떤 단일 팩션도 지속적으로 우위를 점할 수 없게 만드는 설계 $x$** 를 찾는 것이다.

이를 위해서는 “플레이 레벨의 MDP”와 “설계 레벨의 최적화”를 분리해 쓰는 것이 필수적이다.

#### 1.1 본 장의 기여(Contributions)
본 장은 다음을 제공한다.

- **문제의 층위 분리**: 플레이 레벨(고정 MDP)과 설계 레벨(MDP 생성)의 수학적 구분을 명시한다.
- **적용 한계의 논증**: 벨만/DP가 설계 레벨에 직접 적용되기 어려운 이유를 구조적 조건(전이, 마르코프성, 재귀 분해 가능성) 관점에서 정리한다.
- **올바른 문제 분류**: 설계 문제를 확률적 black-box 및 bilevel 최적화로 정식화한다.
...
외부 상태를 $x$로만 둘 경우, 관측 통계 $y(x;\omega)$의 분포는 학습 절차의 초기화/수렴 정도/탐색 정책 등 “학습 상태”에 의해 달라질 수 있다. 특히 warm-start, 메타 적응, 정책 공유 등 실무적 설정에서는 동일한 $x$에 대해 결과 분포가 과거 히스토리에 의존할 수 있다.
...
플레이 레벨의 목표는 시간 누적 보상에 의해 자연스럽게 표현되지만, 설계 레벨의 목표는 통상 “설계의 품질을 단회 평가하여 최소화/최대화”하는 정적 목적 $J(x)$로 주어진다. 이때 할인 누적보상으로 설계 목적을 재작성하는 것은 본질적 이점을 제공하지 않는다.
...
여기서 벨만 방정식은 **내부(플레이) 문제**에서만 사용되며, 외부(설계) 문제는 본질적으로 확률적 black-box 최적화로 남는다.

### 6. 연결: 왜 “공간/거리”가 핵심 설계 변수가 되는가
외부 목적 $J(x)$를 직접 최적화하려면, $x$의 변화가 통계량 $y(x)$에 미치는 효과가 “연속적이며, 저차원이고, 전역적으로 해석 가능”할수록 유리하다. 제2장(`blackbox.md`)에서는 이를 교전 거리 분포 $p_x(d)$의 정합 문제로 환원하고, 비미분적·확률적 목적에서 ES(Score Function Estimator)가 등장하는 과정을 엄밀하게 유도한다. 제3장(`inverse.md`)에서는 더 나아가 “목표 메타”로부터 팩션 파라미터 자체를 생성하는 inverse design 정식화를 제시한다.
