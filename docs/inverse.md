## 제3장 역설계(Inverse Game Design): 목표 메타 기반 팩션·맵 생성

### 초록
제1장에서는 벨만/DP의 적용이 고정된 MDP 위에서만 정당화됨을 논의하였고, 제2장에서는 확률적·비미분 외부 목적함수에서 ES(Score Function Estimator)가 도출되는 과정을 유도하였다. 본 장에서는 목표 메타(Desired Meta)를 만족하는 팩션 파라미터 및 맵/규칙 설계의 역문제를 정식화한다. 구체적으로 (i) 팩션 정체성을 self-play에서 관측되는 통계/분포로 정의하고, (ii) 디자이너의 서술적 의도를 잠재변수 $z_f$로 표현하며, (iii) 생성기 $g(z_f;\phi)$와 설계 $x$를 공동 최적화하여 관측 분포가 목표 분포와 정합되도록 한다. 이때 분포 공간에서의 에너지 최소화 관점이 자연스럽게 도출되며, Wasserstein 거리 $W_2$는 목적함수 구성에 유용한 거리 척도이다.


### 1. 서론
제1장은 벨만/DP의 적용 범위를 “고정된 MDP”라는 조건 아래에서 정리하였고, 제2장은 확률적·비미분 목적함수에서 ES(Score Function Estimator)가 도출되는 과정을 제시하였다. 본 장은 목표 메타를 만족하는 설계 $x$ 및 팩션 파라미터 $\{\theta_f\}$를 역으로 구성하는 문제를 다룬다.
