## 제2장 확률적 블랙박스 최적화: ES 유도와 교전 거리 분포 정합

### 초록
설계 변수 $x$가 게임(MDP) $\mathcal{M}_x$를 정의하고, 그 위에서 학습된 정책 $\pi_x^*$의 self-play 통계 $y(x)$를 통해 밸런스를 평가하는 구조는 본질적으로 확률적 bilevel 최적화이다. 

본 장은 (i) 외부 목적함수의 확률성을 명시하고, (ii) 비미분적 합성 구조 때문에 일반적인 gradient descent가 직접 적용되기 어려움을 설명하며, (iii) ES(Score Function Estimator) 기반의 미분 근사식을 단계별로 유도한다. 

이어서 (iv) 밸런스를 “교전 거리 분포의 정합”으로 환원하여, Wasserstein 거리 $W_2$를 이용한 목적함수 설계를 제안한다.

---

### 키워드
확률적 최적화, black-box 최적화, bilevel 최적화, Evolution Strategy, Score Function Estimator, Optimal Transport, Wasserstein 거리, self-play 평가

---

### 표기 및 약속(Notations and Conventions)
제1장(`bellman.md`)의 공통 표기에 더해, 본 장에서는 다음을 사용한다.

- $\sigma>0$: 가우시안 평활화(smoothing) 강도
- $\epsilon \sim \mathcal{N}(0,I)$: 표준 정규 잡음
- $J_\sigma(x)=\mathbb{E}_\epsilon[J(x+\sigma\epsilon)]$: 평활화된 목적함수

---

### 장 범위
본 장은 설계 목적함수 $J(x)$가 확률적·비미분적일 때의 외부 최적화 기법을 다룬다. 구체적으로 ES(Score Function Estimator)의 유도와, 설계 변수 변화가 메타 통계에 미치는 효과를 “분포 정합”으로 표현하는 한 가지 예시(교전 거리 분포와 $W_2$)를 제시한다. 최종 평가는 제4장(`evaluation.md`)의 승률 중심 프로토콜을 따른다.
...
이 구조에서 강화학습은 $y(x;\omega)$를 계산하는 “내부 평가 절차”로 포함되며, 외부 문제는 $J(x)$에 대한 최적화로 남는다.
...
분포 정합(또는 승률 평탄화)만을 목표로 할 경우, 다음과 같은 퇴화 해가 존재할 수 있다.

- 교전 자체가 거의 발생하지 않도록 설계하여 $p_x(d)$가 의미 없는 분포가 되게 만드는 경우
- 극단적으로 긴/짧은 게임 길이로 승률 분산이 인위적으로 감소하는 경우

따라서 실무적 목적함수 $J(x)$에는 참여도(engagement), 게임 길이 범위, 이벤트 발생률, 난이도, 비용 등의 제약 항을 포함하는 것이 바람직하다.

### 12. 연결: “역설계(inverse design)”로 확장하기
위 논의는 $q_f$(각 팩션이 선호하는 거리대)가 주어졌다고 가정했다. 제3장(`inverse.md`)에서는 “목표 플레이 패턴”으로부터 $q_f$ 및 팩션 파라미터 자체를 학습/생성하는 inverse game design 문제로 확장하며, 분포 정합을 정보기하 관점에서 정리한다.

---

### 13. 실행 가능한 외부 최적화 루프(ES) 설계: 프로토콜·제약·분산 감소를 포함한 최소 형태
앞 절들에서 유도한 ES는 “$J(\cdot)$를 값으로만 평가할 수 있다”는 조건에서 유효하다. 그러나 본 문서가 다루는 $J(x)$는 self-play/RL을 포함하므로, **평가 프로토콜과 제약을 함께 포함**시켜야 퇴화 해를 배제하면서 재현 가능한 최적화를 수행할 수 있다(`evaluation.md`와 연결).

#### 13.1 고정해야 하는 평가 프로토콜(필수)
설계 후보를 공정하게 비교하려면 다음을 명시적으로 고정한다.
